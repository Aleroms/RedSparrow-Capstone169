...
public class AimHelper : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(rayOrigin, PlayerCamera.transform.forward, out hit, 1000)) {//If we hit something with the ray cast
            Vector3 targetDir = hit.point - transform.position;//Figure out the direction off the the target relative to the gun
            float singleStep = speed * Time.deltaTime;//Set the movement speed
            Vector3 newDirection = Vector3.RotateTowards(transform.forward, targetDir, singleStep, 0.0f);//This calculates the new angles the gun needs to rotate in order to point at the target
